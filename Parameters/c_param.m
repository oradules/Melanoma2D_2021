...
xi_max = .15; xi_up = .5; xi_hyp = .5;
nu_max = .15; nu_up = .2; nu_hyp = .8;

%% Spatial Diffusion/Advection Parameters

Dx = [1e-5;1e-5];
chi_m = 2e-4; chi_v = 2e-4;
...
r_shft = .8; r_min = .17;
mLin = cell(2,1);
mLin{1} = (y1 - ((xi_max-r_shft)/(nu_max-r_shft)).*(y2 -r_shft) ...
    -r_shft).*((1 - r_min)*y2 + r_min);
...
phi_0 = .08; phi_up = .45; phi_max = .47;
alpha = 100; w_up_xi = 2; w_up_nu = 35; w_max = 20; w_hyp = .25;
cPreg = (phi_0 ...
    + phi_up*exp(-(w_up_xi*(y1-xi_up).^2+w_up_nu*(y2-nu_up).^2)) ...
    + phi_max*exp(-w_max*((y1-xi_max).^2+(y2-nu_max).^2))).*(1./(1 ...
    +exp(-alpha*((y1-xi_hyp).^2+(y2-nu_hyp).^2-w_hyp^2))));
...
%     - alp_dnu*((y2-alp_y2d).^2) - alp_dxn*((y1-alp_y1d).*(y2-alp_y2d)));
% pDreg{2} = 1-pDreg{1}; %2. complimentary and equal in scope
